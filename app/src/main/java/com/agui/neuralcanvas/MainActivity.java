...
public class MainActivity extends AppCompatActivity
    ...
    private void handleBrainLaunchIntent(Intent intent) {
        ...
        mindMapView.post(() -> {
            ...
            if (openMode) WorkflowModeDialog.show(this, node, focusMode);
        });
    ...
    public void onGraphMutatedByAi() {
        if (mindMapView != null) mindMapView.requestRender();
        scheduleAutoSave();
        try {
            SuggestionFeedbackProfile feedback = dataManager.loadSuggestionFeedbackProfile();
            maybeToast("AI 改图完成｜" + SuggestionFeedbackEngine.buildSummary(feedback));
    ...
    private void loadSavedData() {
        try {
            ...
            if (mindMapView != null && nodes != null && !nodes.isEmpty()) {
    ...
    public void showMoreMenu() {
        MoreMenuDialog.newInstance(id -> MainMenuActionHandler.handle(this, id)).show(getSupportFragmentManager(), "more_menu");
    }
    ...
    public void generateScientificTemplate(ScientificTemplateEngine.TemplateType type) { if (type == null) return; switch (type) { case WOOP: generateWoopFromSelectedNode(); break; case IF_THEN: generateIfThenFromSelectedNode(); break; case DAILY_REVIEW: generateDailyReviewFromSelectedNode(); break; case WEEKLY_REVIEW: generateWeeklyReviewFromSelectedNode(); break; case AAR: generateAarFromSelectedNode(); break; case DECISION_TREE: generateDecisionTreeFromSelectedNode(); break; case PREMORTEM: generatePremortemFromSelectedNode(); break; case EVIDENCE_REVIEW: generateEvidenceReviewFromSelectedNode(); break; case RETRIEVAL_PRACTICE: generateRetrievalPracticeFromSelectedNode(); break; case CONCEPT_DEEPENING: generateConceptDeepeningFromSelectedNode(); break; case TRANSFER_PRACTICE: generateTransferPracticeFromSelectedNode(); break; case WRAP: generateWrapFromSelectedNode(); break; case BAYES_UPDATE: generateBayesUpdateFromSelectedNode(); break; case DSRP_ANALYSIS: generateDsrpAnalysisFromSelectedNode(); break; case REFERENCE_CLASS_FORECAST: generateReferenceClassForecastFromSelectedNode(); break; default: Toast.makeText(this, "暂不支持该模板", Toast.LENGTH_SHORT).show(); } }
    ...
    public void showAiAssistantDialog() { if (isFinishing() || isDestroyed()) return; if (getSupportFragmentManager().findFragmentByTag("ai_assistant_dialog") != null) return; AiAssistantDialog.newInstance().show(getSupportFragmentManager(), "ai_assistant_dialog"); }
    public void showAiAssistantDialogWithPrompt(String presetPrompt) { if (isFinishing() || isDestroyed()) return; if (getSupportFragmentManager().findFragmentByTag("ai_assistant_dialog") != null) return; AiAssistantDialog.newInstance(presetPrompt).show(getSupportFragmentManager(), "ai_assistant_dialog"); }
    public void openAiScienceCoach(String mode, Node node) { if (node == null) { Toast.makeText(this, "请先选中一个节点", Toast.LENGTH_SHORT).show(); return; } mindMapView.selectOnlyNode(node.getId()); String prompt; String normalized = mode == null ? "" : mode.trim(); if ("execution".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.executionCoach(node); else if ("learning".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.learningCoach(node); else if ("decision".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.decisionCoach(node); else if ("redteam".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.redTeam(node); else if ("recommend".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.workflowRecommendation(node); else if ("triage".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.triage(node); else if ("autopilot".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.autopilot(node); else prompt = AiScientificPrompts.gapCheck(node); showAiAssistantDialogWithPrompt(prompt); }
    public void showKnowledgeImportDialog() { if (isFinishing() || isDestroyed()) return; if (getSupportFragmentManager().findFragmentByTag("knowledge_import_dialog") != null) return; KnowledgeImportDialog.newInstance().show(getSupportFragmentManager(), "knowledge_import_dialog"); }
    public void showHelpDialog() { if (isFinishing() || isDestroyed()) return; startActivity(new Intent(this, HelpActivity.class)); }
    public void showThemePicker() { ThemeManager.AppTheme[] themes = ThemeManager.AppTheme.values(); String[] labels = new String[themes.length]; for (int i = 0; i < themes.length; i++) labels[i] = themes[i].label; int current = ThemeManager.getCurrentTheme().ordinal(); new AlertDialog.Builder(this).setTitle("切换主题").setSingleChoiceItems(labels, current, (dialog, which) -> { ThemeManager.setTheme(this, themes[which]); dialog.dismiss(); recreate(); }).setNegativeButton("取消", null).show(); }
    public void openDecisionLab(Node node) { if (node == null) return; mindMapView.selectOnlyNode(node.getId()); openDecisionMatrix(); }
    public void openFocusSession(Node node) { if (node == null) return; mindMapView.selectOnlyNode(node.getId()); openFocusSession(); }
    public void openGraphIntelligence() { openGraphInsights(); }
    public void confirmClearAll() { new AlertDialog.Builder(this).setTitle("清除全部").setMessage("确定删除全部节点和连线吗？此操作会覆盖当前自动保存数据。").setNegativeButton("取消", null).setPositiveButton("确定", (dialog, which) -> { if (mindMapView != null) { mindMapView.clearAll(); saveCurrentDataSilently(); } Toast.makeText(this, "已清除全部内容", Toast.LENGTH_SHORT).show(); }).show(); }
    public void openInboxClarifier() { InboxClarifierDialog.show(this, mindMapView.getNodesInternal(), new InboxClarifierDialog.Callback() { @Override public void onNodeConverted(Node node) { onNodeUpdated(node); mindMapView.requestRender(); scheduleAutoSave(); } @Override public void onBatchFinished() { mindMapView.requestRender(); scheduleAutoSave(); } }); }
    public void openInboxClarifierForSingleNode(Node node) { if (node == null) return; final Node.NodeType[] choices = new Node.NodeType[] { Node.NodeType.TASK, Node.NodeType.ACTION, Node.NodeType.PROJECT, Node.NodeType.IDEA, Node.NodeType.CONCEPT, Node.NodeType.QUESTION, Node.NodeType.RESOURCE, Node.NodeType.DECISION, Node.NodeType.NOTE }; String[] labels = new String[] { "任务", "行动", "项目", "想法", "概念", "问题", "资源", "决策", "笔记" }; new AlertDialog.Builder(this).setTitle("澄清分类").setItems(labels, (dialog, which) -> { Node.NodeType targetType = choices[which]; node.setType(targetType); WorkflowEngine.normalizeNodeForWorkflow(node); if (targetType == Node.NodeType.PROJECT) { node.setProjectId(node.getId()); node.addTags("Project", "InboxConverted"); } else if (targetType == Node.NodeType.TASK || targetType == Node.NodeType.ACTION) { node.addTags("Actionable", "InboxConverted"); } else if (targetType == Node.NodeType.DECISION) { node.addTags("Decision", "InboxConverted"); } else if (targetType == Node.NodeType.CONCEPT || targetType == Node.NodeType.QUESTION || targetType == Node.NodeType.RESOURCE || targetType == Node.NodeType.NOTE) { node.addTags("Learning", "InboxConverted"); } else { node.addTag("InboxConverted"); } onNodeUpdated(node); mindMapView.requestRender(); scheduleAutoSave(); }).setNegativeButton("取消", null).show(); }
    public void openWeeklyReview() { WeeklyReviewDialog.show(this, mindMapView.getNodesInternal(), mindMapView.getConnectionsInternal()); }
    public void openDecisionMatrix() { Node baseNode = getSingleSelectedNode(); if (baseNode == null) { Toast.makeText(this, "请先选中一个决策/项目/目标节点", Toast.LENGTH_SHORT).show(); return; } DecisionMatrixDialog.newInstance(baseNode, mindMapView.getNodesInternal(), mindMapView.getConnectionsInternal(), () -> { mindMapView.requestRender(); scheduleAutoSave(); }).show(getSupportFragmentManager(), "decision_matrix_dialog"); }
    public void openMemoryReview() { MemoryReviewDialog.newInstance(mindMapView.getNodesInternal(), () -> { mindMapView.requestRender(); scheduleAutoSave(); }).show(getSupportFragmentManager(), "memory_review_dialog"); }
    public void openScientificTriage() { Node baseNode = getSingleSelectedNode(); if (baseNode == null) { Toast.makeText(this, "请先选中一个节点", Toast.LENGTH_SHORT).show(); return; } ScientificTriageEngine.TriageReport report = ScientificTriageEngine.analyze(baseNode, mindMapView.getNodesInternal(), mindMapView.getConnectionsInternal()); new AlertDialog.Builder(this).setTitle("节点体检").setMessage(ScientificTriageEngine.buildSummary(report)).setPositiveButton("AI跟进", (d, w) -> openAiScienceCoach(report.suggestedAiMode, baseNode)).setNeutralButton("全量推进", (d, w) -> { mindMapView.selectOnlyNode(baseNode.getId()); runScientificAutopilot(); }).setNegativeButton("关闭", null).show(); }
    public void openExecutionLog() { Node baseNode = getSingleSelectedNode(); if (baseNode == null) { Toast.makeText(this, "请先选中一个任务/行动/项目节点", Toast.LENGTH_SHORT).show(); return; } ExecutionLogDialog.newInstance(baseNode, mindMapView.getNodesInternal(), () -> { mindMapView.requestRender(); scheduleAutoSave(); }).show(getSupportFragmentManager(), "execution_log_dialog"); }
    public void openDecisionFollowThrough() { Node baseNode = getSingleSelectedNode(); if (baseNode == null) { Toast.makeText(this, "请先选中一个决策/目标/项目节点", Toast.LENGTH_SHORT).show(); return; } DecisionFollowThroughDialog.newInstance(baseNode, mindMapView.getNodesInternal(), mindMapView.getConnectionsInternal(), () -> { mindMapView.requestRender(); scheduleAutoSave(); }).show(getSupportFragmentManager(), "decision_follow_dialog"); }
    public void openFocusSession() { Node baseNode = getSingleSelectedNode(); if (baseNode == null) { Toast.makeText(this, "请先选中一个任务/行动节点", Toast.LENGTH_SHORT).show(); return; } FocusSessionDialog.newInstance(this, baseNode).show(getSupportFragmentManager(), "focus_session_dialog"); }
    public void startFocusSession(Node node, int minutes) { if (node == null) { Toast.makeText(this, "未选中节点", Toast.LENGTH_SHORT).show(); return; } FocusSessionEngine.start(this, node, minutes); mindMapView.requestRender(); scheduleAutoSave(); Toast.makeText(this, "已开始专注 Session", Toast.LENGTH_SHORT).show(); }
    public void finishRunningFocusSession(boolean interrupted) { FocusSessionEngine.SessionInfo info = FocusSessionEngine.getCurrent(this); if (info == null) { Toast.makeText(this, "当前没有进行中的 Session", Toast.LENGTH_SHORT).show(); return; } Node node = mindMapView.getNodesInternal().get(info.nodeId); if (node != null) FocusSessionEngine.markTrigger(node, !interrupted); if (interrupted) FocusSessionEngine.interrupt(this); float hours = FocusSessionEngine.finish(this, mindMapView.getNodesInternal(), !interrupted); mindMapView.requestRender(); scheduleAutoSave(); Toast.makeText(this, interrupted ? "Session 已中断，记录时长 " + String.format(java.util.Locale.US, "%.2f", hours) + " 小时" : "Session 已完成，记录时长 " + String.format(java.util.Locale.US, "%.2f", hours) + " 小时", Toast.LENGTH_SHORT).show(); }
    public void openGraphInsights() { Node baseNode = getSingleSelectedNode(); GraphInsightDialog.newInstance(baseNode, mindMapView.getNodesInternal(), mindMapView.getConnectionsInternal()).show(getSupportFragmentManager(), "graph_insight_dialog"); }
    ...
    public void showQuickActionsForNode(Node node) { if (node == null) return; java.util.List<String> actions = QuickActionEngine.getDynamicActions(node); String[] items = actions.toArray(new String[0]); new AlertDialog.Builder(this).setTitle(safeTitle(node)).setItems(items, (dialog, which) -> { String action = actions.get(which); QuickActionEngine.executeDynamicAction(this, node, action); mindMapView.requestRender(); scheduleAutoSave(); }).setNegativeButton("取消", null).show(); }
    public void applyScientificTemplateToNode(Node node, ScientificTemplateEngine.TemplateType type) { if (node == null || type == null) return; mindMapView.selectOnlyNode(node.getId()); switch (type) { case WOOP: applyTemplateResult(ScientificTemplateEngine.generateWoop(node, mindMapView.getNodesInternal())); break; case IF_THEN: applyTemplateResult(ScientificTemplateEngine.generateIfThen(node, mindMapView.getNodesInternal())); break; case DAILY_REVIEW: applyTemplateResult(ScientificTemplateEngine.generateDailyReview(node, mindMapView.getNodesInternal())); break; case WEEKLY_REVIEW: applyTemplateResult(ScientificTemplateEngine.generateWeeklyReview(node, mindMapView.getNodesInternal())); break; case AAR: applyTemplateResult(ScientificTemplateEngine.generateAarReview(node, mindMapView.getNodesInternal())); break; case DECISION_TREE: applyTemplateResult(ScientificTemplateEngine.generateDecisionTree(node, mindMapView.getNodesInternal())); break; case PREMORTEM: applyTemplateResult(ScientificTemplateEngine.generatePremortem(node, mindMapView.getNodesInternal())); break; case EVIDENCE_REVIEW: applyTemplateResult(ScientificTemplateEngine.generateEvidenceReview(node, mindMapView.getNodesInternal())); break; case RETRIEVAL_PRACTICE: applyTemplateResult(ScientificTemplateEngine.generateRetrievalPractice(node, mindMapView.getNodesInternal())); break; case CONCEPT_DEEPENING: applyTemplateResult(ScientificTemplateEngine.generateConceptDeepening(node, mindMapView.getNodesInternal())); break; case TRANSFER_PRACTICE: applyTemplateResult(ScientificTemplateEngine.generateTransferPractice(node, mindMapView.getNodesInternal())); break; case WRAP: applyTemplateResult(ScientificTemplateEngine.generateWrap(node, mindMapView.getNodesInternal())); break; case BAYES_UPDATE: applyTemplateResult(ScientificTemplateEngine.generateBayesUpdate(node, mindMapView.getNodesInternal())); break; case DSRP_ANALYSIS: applyTemplateResult(ScientificTemplateEngine.generateDsrpAnalysis(node, mindMapView.getNodesInternal())); break; case REFERENCE_CLASS_FORECAST: applyTemplateResult(ScientificTemplateEngine.generateReferenceClassForecast(node, mindMapView.getNodesInternal())); break; default: break; } mindMapView.focusNodeById(node.getId()); mindMapView.requestRender(); scheduleAutoSave(); }
    public void editNodeFromQuickAction(Node node) { if (node == null) return; showNodeEditDialog(node); }
    public void deleteNodeFromQuickAction(Node node) { if (node == null) return; new AlertDialog.Builder(this).setTitle("删除节点").setMessage("确定删除“" + safeTitle(node) + "”？").setPositiveButton("删除", (dialog, which) -> { mindMapView.removeNode(node.getId()); mindMapView.requestRender(); scheduleAutoSave(); }).setNegativeButton("取消", null).show(); }
    private String safeTitle(Node node) { if (node == null) return "(空节点)"; String title = node.getTitle(); if (title == null || title.trim().isEmpty()) return "(无标题)"; return title.trim(); }
    ...
    private void generateTransferPracticeFromSelectedNode() { Node baseNode = getSingleSelectedNode(); if (baseNode == null) { Toast.makeText(this, "请先单击选中一个知识/方法节点", Toast.LENGTH_SHORT).show(); return; } applyTemplateResult(ScientificTemplateEngine.generateTransferPractice(baseNode, mindMapView.getNodesInternal())); mindMapView.focusNodeById(baseNode.getId()); Toast.makeText(this, "已生成应用迁移子图", Toast.LENGTH_SHORT).show(); }
    ...
    private void applyTemplateResult(ScientificTemplateEngine.TemplateResult result) { if (result == null) return; Node baseNode = getSingleSelectedNode(); TemplatePostProcessor.postProcess(baseNode, result, mindMapView.getNodesInternal()); for (Node node : result.createdNodes) mindMapView.addNode(node); for (Connection connection : result.createdConnections) mindMapView.addConnection(connection); TemplateStateSynchronizer.synchronize(baseNode, result, mindMapView.getNodesInternal(), mindMapView.getConnectionsInternal()); mindMapView.requestRender(); scheduleAutoSave(); }
    public void showNodeEditDialog(Node node) { DialogFragment dialog = NodeEditDialog.newInstance(node, mindMapView); dialog.show(getSupportFragmentManager(), "node_edit_dialog"); }
    ...
    private void saveCurrentDataSilently() { try { final Map<String, Node> nodes = mindMapView.getNodes(); final Map<String, Connection> connections = mindMapView.getConnections(); ioExecutor.execute(() -> { try { dataManager.saveMindMap(nodes, connections); } catch (Exception e) { e.printStackTrace(); } }); } catch (Exception e) { e.printStackTrace(); } }
    @Override public void onNodeUpdated(Node node) { if (mindMapView != null) mindMapView.requestRender(); scheduleAutoSave(); maybeToast("节点已更新"); }
    @Override public void onNodeDeleted(Node node) { scheduleAutoSave(); maybeToast("节点已删除"); }
    @Override public void onSearch(String keyword, List<Node.NodeType> types, boolean highlight) { if (mindMapView == null) return; mindMapView.search(keyword, types, highlight); int resultCount = mindMapView.getSearchResultCount(); maybeToast(resultCount > 0 ? "找到 " + resultCount + " 个匹配节点" : "未找到匹配节点"); }
    @Override public void onClearSearch() { if (mindMapView != null) mindMapView.clearSearch(); maybeToast("搜索已清除"); }
    @Override public void onDataChanged() { scheduleAutoSave(); }
    @Override protected void onResume() { super.onResume(); maybeShowPendingBrainGuidance(false); try { BrainAutopilotSettings settings = dataManager == null ? null : dataManager.loadAutopilotSettings(); if (settings != null && settings.isEnabled() && settings.isApiAutopilotEnabled() && settings.isInAppPulseOnResume()) BrainAutopilotScheduler.requestImmediatePulse(this); } catch (Exception ignored) {} }
    ...
    private void maybeToast(String message) { long now = System.currentTimeMillis(); if (now - lastToastAt < 500L) return; lastToastAt = now; Toast.makeText(this, message, Toast.LENGTH_SHORT).show(); }
}
