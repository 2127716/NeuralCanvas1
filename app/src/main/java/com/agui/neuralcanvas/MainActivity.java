...
public class MainActivity extends AppCompatActivity
    ...
    public void runScientificEnhancement() {
        ...
        ScientificEnhancementEngine.EnhancementResult result = ScientificEnhancementEngine.enhance(
                baseNode, mindMapView.getNodesInternal(), mindMapView.getConnectionsInternal());
        applyTemplateResult(result.templateResult);
        if (result.touchedBaseNode) onNodeUpdated(baseNode);
    ...
    public void openAiScienceCoach(String mode, Node node) {
        ...
        if ("execution".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.executionCoach(node);
        else if ("learning".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.learningCoach(node);
        else if ("decision".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.decisionCoach(node);
        else if ("redteam".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.redTeam(node);
        else if ("recommend".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.workflowRecommendation(node);
        else if ("triage".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.triage(node);
        else if ("autopilot".equalsIgnoreCase(normalized)) prompt = AiScientificPrompts.autopilot(node);
        else prompt = AiScientificPrompts.gapCheck(node);
    ...
    public void openExecutionLog() {
        ...
        ExecutionLogDialog.newInstance(baseNode, mindMapView.getNodesInternal(), () -> {
            mindMapView.requestRender();
            scheduleAutoSave();
    ...
    public void openDecisionFollowThrough() {
        ...
        DecisionFollowThroughDialog.newInstance(baseNode, mindMapView.getNodesInternal(), mindMapView.getConnectionsInternal(), () -> {
            mindMapView.requestRender();
            scheduleAutoSave();
    ...
    public void applyScientificTemplateToNode(Node node, ScientificTemplateEngine.TemplateType type) {
        ...
        switch (type) {
            case WOOP:
                applyTemplateResult(ScientificTemplateEngine.generateWoop(node, mindMapView.getNodesInternal())); break;
            case IF_THEN:
                applyTemplateResult(ScientificTemplateEngine.generateIfThen(node, mindMapView.getNodesInternal())); break;
            case DAILY_REVIEW:
                applyTemplateResult(ScientificTemplateEngine.generateDailyReview(node, mindMapView.getNodesInternal())); break;
            case WEEKLY_REVIEW:
                applyTemplateResult(ScientificTemplateEngine.generateWeeklyReview(node, mindMapView.getNodesInternal())); break;
            case AAR:
                applyTemplateResult(ScientificTemplateEngine.generateAarReview(node, mindMapView.getNodesInternal())); break;
            case DECISION_TREE:
                applyTemplateResult(ScientificTemplateEngine.generateDecisionTree(node, mindMapView.getNodesInternal())); break;
            case PREMORTEM:
                applyTemplateResult(ScientificTemplateEngine.generatePremortem(node, mindMapView.getNodesInternal())); break;
            case EVIDENCE_REVIEW:
                applyTemplateResult(ScientificTemplateEngine.generateEvidenceReview(node, mindMapView.getNodesInternal())); break;
            case RETRIEVAL_PRACTICE:
                applyTemplateResult(ScientificTemplateEngine.generateRetrievalPractice(node, mindMapView.getNodesInternal())); break;
            case CONCEPT_DEEPENING:
                applyTemplateResult(ScientificTemplateEngine.generateConceptDeepening(node, mindMapView.getNodesInternal())); break;
            case TRANSFER_PRACTICE:
                applyTemplateResult(ScientificTemplateEngine.generateTransferPractice(node, mindMapView.getNodesInternal())); break;
            case WRAP:
                applyTemplateResult(ScientificTemplateEngine.generateWrap(node, mindMapView.getNodesInternal())); break;
            case BAYES_UPDATE:
                applyTemplateResult(ScientificTemplateEngine.generateBayesUpdate(node, mindMapView.getNodesInternal())); break;
            case DSRP_ANALYSIS:
                applyTemplateResult(ScientificTemplateEngine.generateDsrpAnalysis(node, mindMapView.getNodesInternal())); break;
            case REFERENCE_CLASS_FORECAST:
                applyTemplateResult(ScientificTemplateEngine.generateReferenceClassForecast(node, mindMapView.getNodesInternal())); break;
            default: break;
        }
    ...
    private Node getSingleSelectedNode() {
        ...
        return mindMapView.getNodesInternal().get(selectedIds.get(0));
    }
    ...
    private void saveCurrentDataSilently() {
        try {
            ...
            ioExecutor.execute(() -> {
                try { dataManager.saveMindMap(nodes, connections); }
    ...
    public void onSearch(String keyword, List<Node.NodeType> types, boolean highlight) {
        ...
        maybeToast(resultCount > 0 ? "找到 " + resultCount + " 个匹配节点" : "未找到匹配节点");
    }

    @Override
    public void onClearSearch() {
        if (mindMapView != null) mindMapView.clearSearch();
        maybeToast("搜索已清除");
    }
