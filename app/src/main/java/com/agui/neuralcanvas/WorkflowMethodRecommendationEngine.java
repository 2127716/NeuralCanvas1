...
public final class WorkflowMethodRecommendationEngine {
    ...
    public static Analysis analyze(Node node,
        ...
        boolean hasTrigger = !WorkflowEngine.isBlank(node.getTriggerCondition()) || hasNearbyType(node, nodes, Node.NodeType.TRIGGER);
        ...
        if (executionLike) {
            if (!hasTrigger) {
                ...
                analysis.recommendations.add(new Recommendation("补 If-Then", "把“想做”变成“遇到什么情境就立刻做什么”。", "template:IF_THEN", "execution", 100));
                execScore += 3;
            }
            if (!hasObstacle) {
                analysis.gaps.add("执行链缺少障碍分析");
                analysis.recommendations.add(new Recommendation("补 WOOP", "把愿望、障碍和计划连起来，减少卡住后停摆。", "template:WOOP", "execution", 94));
                analysis.recommendations.add(new Recommendation("AI 执行补全", "自动补最小下一步、触发条件和完成判据。", "action:AI_EXECUTION_PATCH", "execution", 92));
                execScore += 3;
            }
            if (!hasAction) {
                analysis.gaps.add("当前节点还没有清晰的下一步行动");
                analysis.recommendations.add(new Recommendation("执行补强", "先生成最小下一步，再进入专注执行。", "action:SCIENTIFIC_ENHANCEMENT", "execution", 98));
                execScore += 4;
            }
            if (!hasReview) {
                analysis.gaps.add("执行后缺少反馈闭环");
                analysis.recommendations.add(new Recommendation("补每日复盘", "把今天推进、卡点、经验和明日最小下一步接起来。", "template:DAILY_REVIEW", "execution", 84));
                execScore += 2;
            }
            if (node.getType() == Node.NodeType.PROJECT && !hasReview) {
                analysis.recommendations.add(new Recommendation("补每周复盘", "项目类节点更适合建立周期性调整回路。", "template:WEEKLY_REVIEW", "execution", 88));
            }
        }
        ...
        if (decisionLike) {
            if (!hasOption) {
                ...
                analysis.recommendations.add(new Recommendation("补决策树", "先把候选方案和准则摊开，不要直接拍板。", "template:DECISION_TREE", "decision", 99));
                analysis.recommendations.add(new Recommendation("进入决策实验室", "直接做多标准比较。", "action:DECISION_LAB", "decision", 94));
                decisionScore += 4;
            }
            if (!hasEvidence || node.getEvidenceStrength() < 0.45f) {
                analysis.gaps.add("当前决策证据不足或证据强度偏低");
                analysis.recommendations.add(new Recommendation("补证据审查", "把支持证据、反对证据和待验证假设拉出来。", "template:EVIDENCE_REVIEW", "decision", 96));
                decisionScore += 3;
            }
            if (!hasRisk) {
                analysis.gaps.add("当前决策还没有失败预演或风险前置");
                analysis.recommendations.add(new Recommendation("补 Premortem", "先假设失败，再倒推原因和预警信号。", "template:PREMORTEM", "decision", 90));
                decisionScore += 3;
            }
            if (node.getConfidence() > 0.72f && (node.getEvidenceStrength() < 0.50f || !hasEvidence)) {
                analysis.gaps.add("置信度偏高，但证据不足");
                analysis.recommendations.add(new Recommendation("补 Bayes 更新", "把先验、基率、支持证据和反向证据写清楚。", "template:BAYES_UPDATE", "decision", 97));
            }
            if (node.getType() == Node.NodeType.PROJECT || node.getType() == Node.NodeType.GOAL) {
                analysis.recommendations.add(new Recommendation("补参考类预测", "用外部视角修正时间/成功率估计。", "template:REFERENCE_CLASS_FORECAST", "decision", 82));
            }
            if (hasOption || hasEvidence || hasRisk) {
                analysis.recommendations.add(new Recommendation("决策落地", "把当前判断转成承诺动作、止损线和跟进节点。", "action:DECISION_FOLLOW_THROUGH", "decision", 78));
            }
        }
        ...
        if (learningLike) {
            if (!hasQuestion) {
                ...
                analysis.recommendations.add(new Recommendation("补检索练习", "不用看原文回忆定义、机制和关键点。", "template:RETRIEVAL_PRACTICE", "learning", 100));
                learningScore += 4;
            }
            if (!hasResource) {
                analysis.gaps.add("当前知识结构缺少例子或来源");
                analysis.recommendations.add(new Recommendation("补概念深化", "把定义、例子、反例和易错点补齐。", "template:CONCEPT_DEEPENING", "learning", 92));
                learningScore += 3;
            }
            if (!hasExperiment) {
                analysis.gaps.add("当前知识结构缺少迁移或验证任务");
                analysis.recommendations.add(new Recommendation("补迁移练习", "换个场景应用它，检验自己是不是会用了。", "template:TRANSFER_PRACTICE", "learning", 88));
                learningScore += 3;
            }
            if (!hasReview && WorkflowEngine.isBlank(node.getReviewAt())) {
                analysis.gaps.add("当前知识结构缺少复习安排");
                analysis.recommendations.add(new Recommendation("打开记忆复习", "为知识节点建立复习与检索节奏。", "action:MEMORY_REVIEW", "learning", 80));
            }
        }

        if (execScore >= decisionScore && execScore >= learningScore) analysis.dominantMode = "execution";
        else if (decisionScore >= learningScore) analysis.dominantMode = "decision";
        else analysis.dominantMode = "learning";
    ...
    public static String buildReadableReport(Analysis analysis) {
        ...
        if (!analysis.gaps.isEmpty()) {
            sb.append("\n\n当前主要缺口：");
            for (String item : analysis.gaps) sb.append("\n• ").append(item);
        }
        if (!analysis.recommendations.isEmpty()) {
            sb.append("\n\n推荐顺序：");
            int limit = Math.min(4, analysis.recommendations.size());
    ...
    public static void execute(MainActivity activity, Node node, String actionId) {
        ...
        if (actionId.startsWith("template:")) {
            ...
            try {
                activity.applyScientificTemplateToNode(node, ScientificTemplateEngine.TemplateType.valueOf(typeName));
        ...
        switch (actionId) {
            ...
            case "action:QUICK_FIX":
                ...
                FocusGuideDialog.show(activity, NodeFocusGuideEngine.buildForFix(activity, node, fixResult));
                return;
            default:
                break;
        }
    ...
    private static boolean hasNearbyType(Node baseNode, Map<String, Node> existingNodes, Node.NodeType... targetTypes) {
        if (baseNode == null || existingNodes == null || targetTypes == null || targetTypes.length == 0) return false;
        for (Node node : existingNodes.values()) {
            ...
            if (!matched) continue;
            if (sameOwner(baseNode, node) || distance(baseNode, node) <= 920f) return true;
        }
        return false;
    }
    ...
    private static void sortAndDedupe(List<Recommendation> source) {
        ...
        for (int i = source.size() - 1; i >= 0; i--) {
            ...
            if (seen.contains(key)) source.remove(i);
            else seen.add(key);
        }
    }
}
